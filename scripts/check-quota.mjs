...
function updateGroup(groups, group, model, remainingFraction, resetTime) {
  if (!groups[group]) {
    groups[group] = [];
  }
...
function printGroup(label, entries) {
  ...
  console.log(`   ${label}:`);
  const sortedEntries = [...entries].sort((a, b) => a.model.localeCompare(b.model));
  for (const entry of sortedEntries) {
    ...
    console.log(`      - ${entry.model}: ${status}${suffix}`);
  }
}
  ...
  for (const { account, index } of selected) {
    ...
    try {
      ...
      for (const [modelName, info] of Object.entries(models)) {
        ...
        updateGroup(groups, group, modelName, remaining, info.quotaInfo.resetTime);
      }
