...
describe("AccountManager", () => {
  ...
  describe("soft quota threshold", () => {
    it("skips account over soft quota threshold in sticky mode", () => {
      ...
      const manager = new AccountManager(undefined, stored);
      manager.updateQuotaCache(0, { claude: [{ model: "claude", remainingFraction: 0.05 }] });
    ...
    it("allows account under soft quota threshold", () => {
      ...
      const manager = new AccountManager(undefined, stored);
      manager.updateQuotaCache(0, { claude: [{ model: "claude", remainingFraction: 0.15 }] });
    ...
    it("threshold of 100 disables soft quota protection", () => {
      ...
      const manager = new AccountManager(undefined, stored);
      manager.updateQuotaCache(0, { claude: [{ model: "claude", remainingFraction: 0.01 }] });
    ...
    it("returns null when all accounts over threshold", () => {
      ...
      const manager = new AccountManager(undefined, stored);
      manager.updateQuotaCache(0, { claude: [{ model: "claude", remainingFraction: 0.05 }] });
      manager.updateQuotaCache(1, { claude: [{ model: "claude", remainingFraction: 0.08 }] });
    ...
    it("handles remainingFraction of 0 (fully exhausted)", () => {
      ...
      const manager = new AccountManager(undefined, stored);
      manager.updateQuotaCache(0, { claude: [{ model: "claude", remainingFraction: 0 }] });
    ...
    it("fails open when cachedQuotaUpdatedAt is missing", () => {
      ...
      acc.cachedQuota = { claude: [{ model: "claude", remainingFraction: 0.05 }] };
