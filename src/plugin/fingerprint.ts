...
import { getAntigravityVersion, mapRuntimePlatformToMetadataPlatform } from "../constants";
...
export function generateFingerprint(): Fingerprint {
  ...
  const detectedPlatform = mapRuntimePlatformToMetadataPlatform(platform);
  const matchingPlatform = PLATFORMS.includes(detectedPlatform)
    ? detectedPlatform
...
export function collectCurrentFingerprint(): Fingerprint {
  ...
  const matchingPlatform = mapRuntimePlatformToMetadataPlatform(platform);
