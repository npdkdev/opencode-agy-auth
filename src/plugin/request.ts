...
export function prepareAntigravityRequest(
  ...
  modelLimits: ModelLimits = getDefaultModelLimits(),
): {
  request: RequestInfo;
  init: RequestInit;
  streaming: boolean;
      ...
      if (isWrapped) {
        ...
        if (isClaude) {
          ...
          if (Array.isArray(requestPayload.contents)) {
            requestPayload.contents = filterThinkingFromHistory(
        ...
        if (isClaude && headerStyle === "antigravity") {
          ...
          if (Array.isArray(requestPayload.contents)) {
            ...
            requestPayload.contents = pruneOldTurns(
              requestPayload.contents,
              estimatedTokens,
              HARD_LIMIT,
            );
