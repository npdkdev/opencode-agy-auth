...
export interface ManagedAccount {
  ...
  cachedQuota?: Partial<Record<QuotaGroup, QuotaModelEntry[]>>;
...
interface LegacyQuotaGroupSummary {
...
function isLegacyQuotaGroupSummary(value: unknown): value is LegacyQuotaGroupSummary {
  if (!value || typeof value !== "object") {
    return false;
  }
...
function isOverSoftQuotaThreshold(
  ...
  if (minFraction == null) return false;

  const remainingFraction = Math.max(
    0,
    Math.min(1, minFraction),
  );
  ...
  if (isOverThreshold) {
    ...
    if (resetTime) {
      ...
      if (Number.isFinite(resetMs) && nowMs() < resetMs) {
        ...
        debugLogToFile(
          `[SoftQuota] STRICT LOCK ${accountLabel}: ${quotaGroup} usage ${usedPercent.toFixed(1)}% >= ${thresholdPercent}%. Waiting for reset at ${resetTime}`,
        );
        return true;
      }
    }
...
export class AccountManager {
  ...
  updateQuotaCache(
    accountIndex: number,
    quotaGroups: Partial<Record<QuotaGroup, QuotaModelEntry[]>>,
  ): void {
    const account = this.accounts[accountIndex];
    if (account) {
