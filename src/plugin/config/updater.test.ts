...
describe("updateOpencodeConfig", () => {
  ...
  test("creates new config with default structure when file does not exist", async () => {
    ...
    expect(writtenConfig.plugin).toContain("opencode-agy-auth@latest");
  ...
  test("preserves $schema and other top-level config keys", async () => {
    const existingConfig = {
      $schema: "https://opencode.ai/config.json",
      plugin: ["opencode-agy-auth@latest", "other-plugin"],
      theme: "dark",
  ...
  test("migrates legacy plugin ID to the new package ID", async () => {
    const existingConfig = {
      plugin: ["opencode-ag-auth@1.6.3", "other-plugin"],
      provider: {},
    };
    fs.writeFileSync(configPath, JSON.stringify(existingConfig));
    ...
    const writtenConfig = JSON.parse(fs.readFileSync(configPath, "utf-8"));
    expect(writtenConfig.plugin).toContain("opencode-agy-auth@1.6.3");
    expect(writtenConfig.plugin).not.toContain("opencode-ag-auth@1.6.3");
  });
  ...
  test("does not duplicate plugin if different version present", async () => {
    const existingConfig = {
      plugin: ["opencode-agy-auth@beta", "other-plugin"],
      provider: {},
    };
    fs.writeFileSync(configPath, JSON.stringify(existingConfig));
    ...
    expect(writtenConfig.plugin).toContain("opencode-agy-auth@beta");
  });
