...
const PLUGIN_ID = "opencode-agy-auth";
const LEGACY_PLUGIN_ID = "opencode-ag-auth";
const PLUGIN_NAME = `${PLUGIN_ID}@latest`;
...
export async function updateOpencodeConfig(
  ...
  try {
    ...
    config.plugin = config.plugin.map((entry) => {
      if (!entry.includes(LEGACY_PLUGIN_ID) || entry.includes(PLUGIN_ID)) {
        return entry;
      }
      return entry.replace(LEGACY_PLUGIN_ID, PLUGIN_ID);
    });

    // Check if plugin is already in the list (any version)
    const hasPlugin = config.plugin.some((p) => p.includes(PLUGIN_ID));
    if (!hasPlugin) {
