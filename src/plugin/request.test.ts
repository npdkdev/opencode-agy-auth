...
describe("request.ts", () => {
  ...
  describe("prepareAntigravityRequest", () => {
    ...
    it("sets Content-Type: application/json for antigravity headerStyle", () => {
      const result = prepareAntigravityRequest(
        "https://generativelanguage.googleapis.com/v1beta/models/claude-opus-4-6-thinking:generateContent",
        { method: "POST", body: JSON.stringify({ contents: [] }) },
        mockAccessToken,
    ...
    it("deletes empty X-Goog-Api-Key for antigravity headerStyle", () => {
      const result = prepareAntigravityRequest(
        "https://generativelanguage.googleapis.com/v1beta/models/claude-opus-4-6-thinking:generateContent",
        { method: "POST", body: JSON.stringify({ contents: [] }), headers: { "X-Goog-Api-Key": "  " } },
        mockAccessToken,
        mockProjectId,
        undefined,
        "antigravity"
    ...
    it("preserves non-empty X-Goog-Api-Key for antigravity headerStyle", () => {
      const result = prepareAntigravityRequest(
        "https://generativelanguage.googleapis.com/v1beta/models/claude-opus-4-6-thinking:generateContent",
        { method: "POST", body: JSON.stringify({ contents: [] }), headers: { "X-Goog-Api-Key": "valid-key" } },
        mockAccessToken,
        mockProjectId,
        undefined,
        "antigravity"
      ...
      const headers = result.init.headers as Headers;
      expect(headers.get("X-Goog-Api-Key")).toBe("valid-key");
    });

    it("does NOT set X-Goog-Api-Client or Client-Metadata for antigravity headerStyle", () => {
      const result = prepareAntigravityRequest(
        "https://generativelanguage.googleapis.com/v1beta/models/claude-opus-4-6-thinking:generateContent",
        { method: "POST", body: JSON.stringify({ contents: [] }) },
        mockAccessToken,
