 ...
 */
export function filterThinkingFromHistory(
  contents: unknown[],
  keepThinking: boolean
): unknown[] {
  if (!Array.isArray(contents)) return contents

  return contents.map((turn: unknown) => {
    if (!turn || typeof turn !== "object") return turn
    const t = turn as Record<string, unknown>
    if (!Array.isArray(t["parts"])) return turn

    const filteredParts = (t["parts"] as unknown[])
      .map((part: unknown) => {
        ...
        if (p["thought"] === true) {
          if (!keepThinking) return null // Strip entire part
...
export function parseContextLimitFromError(errorMessage: string): { maxTokens?: number; actualTokens?: number } | null {
 ...
 */
export function truncateOldToolResponses(
  contents: unknown[],
  maxChars = MAX_TOOL_RESPONSE_CHARS,
  keepFullTurns = MAX_HISTORY_TURNS_WITH_FULL_TOOLS,
): unknown[] {
  ...
  return contents.map((turn: unknown, index: number) => {
    if (index >= cutoffIndex) return turn
    if (!turn || typeof turn !== "object") return turn
    const t = turn as Record<string, unknown>
    if (!Array.isArray(t["parts"])) return turn
    ...
    return {
      ...t,
      parts: (t["parts"] as unknown[]).map((part: unknown) => {
        if (!part || typeof part !== "object") return part
        const p = part as Record<string, unknown>
        if (!p["functionResponse"]) return part

        const fr = p["functionResponse"] as Record<string, unknown>
        const responseObj = fr["response"] as Record<string, unknown> | undefined
...
export function pruneOldTurns(
  ...
  const turnsToRemove = Math.min(
    Math.ceil(excessTokens / tokensPerTurn),
    maxRemovable,
  )
