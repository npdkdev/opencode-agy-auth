...
async function runAutoCompactCommand(
  client: PluginClient,
  sessionId: string,
  directory: string,
  timeoutMs: number = AUTO_COMPACT_TIMEOUT_MS,
): Promise<void> {
  ...
  if (sessionId) {
    try {
      ...
      return createSyntheticErrorResponse(
        "✅ Context was too long and has been automatically compacted.\n\nPlease **resend your last message** to continue.",
        model,
      );
    } catch {
      return createSyntheticErrorResponse(
        "⚠️ Context too long and auto-compact failed.\n\nPlease run **/compact** manually, then resend your message.",
        model,
      );
    }
  }
...
let cachedModelLimits: ModelLimits = getDefaultModelLimits();
                ...
                while (!shouldSwitchAccount) {
                  ...
                  for (
                    ...
                    try {
                        ...
                        if (
                          ...
                          if (capacityRetryCount < 3) {
                            ...
                            const remainingEndpoints =
                              ANTIGRAVITY_ENDPOINT_FALLBACKS.slice(i + 1).filter(
                                (ep) =>
                                  headerStyle !== "gemini-cli" ||
                            ...
                            if (remainingEndpoints.length === 0) {
                              pushDebug(
                                `All capacity retries exhausted across all endpoints for ${rateLimitReason}. Will return 503 for fallback_models.`,
                              );
                              ...
                              pushDebug(
                                `Max capacity retries (3) exhausted for endpoint ${currentEndpoint}, trying next endpoint...`,
                              );
                      ...
                      if (!response.ok) {
                        ...
                        if (response.status === 400) {
                          ...
                          const isContextOverflow =
                            bodyTextLower.includes("prompt is too long") ||
                          ...
                          if (parsedLimit?.maxTokens) {
                            ...
                            pushDebug(
                              `learned context limit ${family}=${parsedLimit.maxTokens}`,
                            );
                          }
                          ...
                          if (
                            ...
                            await showToast(
                              "Context too long. Auto-compacting and retrying once...",
                              "warning",
                            );

                            let compactFailed = false;
                            try {
                              await runAutoCompactCommand(
                                client,
                                currentOpenCodeSessionId,
                                directory,
                            ...
                            if (!compactFailed) {
                              ...
                              const retryParsedLimit = parseContextLimitFromError(
                                retryBodyText,
                              );
                            ...
                            return createSyntheticErrorResponse(
                              `[Antigravity Error] Context is still too long after automatic recovery.${compactHint}${limitHint}${actualHint}\n\nRun /compact manually, then resend your last message.`,
                              prepared.requestedModel,
                            );
                          }
                          ...
                          if (isContextOverflow) {
                            ...
                            const actualHint = parsedLimit?.actualTokens
                              ...
                              : "";
                            return createSyntheticErrorResponse(
                              `[Antigravity Error] Context is too long for this model.${limitHint}${actualHint}\n\nUse /compact to reduce context size, then retry your request.`,
                              prepared.requestedModel,
                            );
                          }
                        }
                      ...
                      if (
                        transformedResponse.headers.get(
                          "x-antigravity-thinking-recovery",
                        ) === "needed" &&
                        !hasRetriedThinkingStrip
                      ) {
                        hasRetriedThinkingStrip = true;
                        log.debug(
                          "Silent retry: stripping thinking blocks and retrying request",
                        );

                        if (typeof prepared.init.body === "string") {
                          try {
                            const retryPayload: unknown = JSON.parse(
                              prepared.init.body,
                            );
                            ...
                            if (
                              ...
                              const retryPayloadBody =
                                retryPayload as Record<string, unknown>;
                            ...
                            pushDebug(
                              "thinking-recovery: failed to parse request body for silent retry",
                            );
                          }
                        }
                      }
                  ...
                  if (isCapacityExhausted) break;
                } // end headerStyleLoop

                if (isCapacityExhausted && lastFailure) {
                  pushDebug(
                    `Capacity exhausted for model ${model}. Returning 503 to trigger fallback_models.`,
                  );
                  await showToast(
                    `🔄 ${model} capacity exhausted. Triggering model fallback...`,
                    "warning",
                  );
                  return transformAntigravityResponse(
              ...
              if (inputs) {
                      ...
                      (acc, idx) => {
                        ...
                        if (acc.verificationRequired) {
                          ...
                          if (acc.cachedQuota) {
                            ...
                            if (
                              typeof claudeMinFraction === "number" &&
                              claudeMinFraction <= 0 &&
                              !familyResets["claude"]
                            ) {
                            ...
                            const proExhausted =
                              typeof geminiProMin === "number" &&
                              geminiProMin <= 0;
                            const flashExhausted =
                              typeof geminiFlashMin === "number" &&
                              geminiFlashMin <= 0;
                        ...
                        if (res.quota?.groups) {
                          ...
                          if (acc) {
                            ...
                            if (claudeQuota.length > 0) {
                              const claudeMinFraction = getGroupMinFraction(
                                claudeQuota,
                              );
                              const claudeReset = getGroupEarliestReset(
                                claudeQuota,
                              );
                              if (
                                typeof claudeMinFraction === "number" &&
                            ...
                            const geminiProMin = getGroupMinFraction(geminiPro);
                            const geminiFlashMin = getGroupMinFraction(
                              geminiFlash,
                            );
                            ...
                            if (geminiExhausted && flashExhausted) {
                              ...
                              const proResetTime = getGroupEarliestReset(
                                geminiPro,
                              );
                              const flashResetTime = getGroupEarliestReset(
                                geminiFlash,
                              );
                              const proReset = proResetTime
                ...
                while (accounts.length < MAX_OAUTH_ACCOUNTS) {
                  ...
                  if (result.type === "failed") {
                    ...
                    console.warn(
                      `[opencode-agy-auth] Skipping failed account ${accounts.length + 1}: ${result.error}`,
                    );
                    break;
                  }
