...
export function getAntigravityHeaders(): HeaderSet & { "Client-Metadata": string } {
  return {
    ...
    "Client-Metadata": `{"ideType":"ANTIGRAVITY","platform":"${mapRuntimePlatformToMetadataPlatform()}","pluginType":"GEMINI"}`,
  };
}
...
export function mapRuntimePlatformToMetadataPlatform(runtimePlatform: NodeJS.Platform = process.platform): "WINDOWS" | "MACOS" | "LINUX" {
...
export function detectPlatformString(runtimePlatform: NodeJS.Platform = process.platform, runtimeArch: string = process.arch): string {
  return `${runtimePlatform}/${runtimeArch}`;
}

export const ANTIGRAVITY_PLATFORM = detectPlatformString();
export const ANTIGRAVITY_PLATFORMS = ["windows/amd64", "darwin/arm64", "darwin/amd64", "linux/x64", "linux/arm64"] as const;
...
export function getRandomizedHeaders(style: HeaderStyle, model?: string): HeaderSet {
  ...
  const platform = ANTIGRAVITY_PLATFORM;
  const metadataPlatform = mapRuntimePlatformToMetadataPlatform();
  return {
...
const learnedLimits: Map<string, number> = new Map();

export function updateLearnedLimit(modelFamily: string, maxTokens: number): void {
  const current = learnedLimits.get(modelFamily);
  if (!current || maxTokens < current) {
    learnedLimits.set(modelFamily, maxTokens);
  }
}

export function getLearnedLimit(modelFamily: string): number | undefined {
  return learnedLimits.get(modelFamily);
}

export interface ModelLimits {
  claude_context_limit: number;
  claude_thinking_budget_max: number;
  gemini_context_limit: number;
  last_discovered_at?: string;
}

export function getDefaultModelLimits(): ModelLimits {
  return {
    claude_context_limit: DEFAULT_CLAUDE_CONTEXT_LIMIT,
    claude_thinking_budget_max: DEFAULT_CLAUDE_THINKING_BUDGET,
    gemini_context_limit: DEFAULT_GEMINI_CONTEXT_LIMIT,
  };
}
