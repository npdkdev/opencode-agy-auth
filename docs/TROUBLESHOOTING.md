...
rm -rf ~/.cache/opencode/node_modules/opencode-agy-auth
rm -rf ~/.bun/install/cache/opencode-agy-auth*
opencode auth login
```
...
Remove-Item "$env:LOCALAPPDATA\opencode\Cache\node_modules\opencode-agy-auth" -Recurse -Force -ErrorAction SilentlyContinue
Remove-Item "$env:USERPROFILE\.bun\install\cache\opencode-agy-auth*" -Recurse -Force -ErrorAction SilentlyContinue
opencode auth login
```
...
3. Update to latest beta: `"plugin": ["opencode-agy-auth@beta"]`
...
If you're seeing unusual rate limit behavior, please share in a [GitHub issue](https://github.com/npdkdev/opencode-agy-auth/issues):
...
Open an issue on [GitHub](https://github.com/npdkdev/opencode-agy-auth/issues).
