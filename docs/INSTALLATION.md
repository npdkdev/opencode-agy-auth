...
This guide provides comprehensive installation instructions for **OpenCode AGY Auth** for both human users and AI agents.
...
2.  Add `opencode-agy-auth` to the `plugin` array:
...
1.  **Clone the repository:**
    ```bash
    git clone https://github.com/npdkdev/opencode-agy-auth.git
    cd opencode-agy-auth
    ```
...
```markdown
# OpenCode AGY Auth Installation Protocol
...
2. **Add Plugin**:
   ...
   - Append "opencode-agy-auth@latest" to the "plugin" array.
   - Example state:
     {
       "plugin": ["opencode-agy-auth@latest"]
     }
...
3. **Configure Models**:
   ...
   - Define models mapping to "opencode-agy-auth" capabilities.
...
try {
  ...
  const hasPlugin = config.plugin.some(p => p.includes('opencode-agy-auth'));
  
  if (hasPlugin) {
    console.log('✅ OpenCode AGY Auth plugin is configured.');
  } else {
